...
Rcpp::List cluster_inbreeding_coef_cpp(Rcpp::List args, Rcpp::List args_functions, Rcpp::List args_progress) {
  ...
  for (int step = 0; step < steps; step++) {
    ...
    //-------------------------------
      ...
      for (int ind = 0; ind < n; ind++) {
        // gradient
        mpartial = 2 * gendist[ind] * geodist[ind] * ((indclst1_fis[ind] + indclst2_fis[ind])/2) *
                   exp(-geodist[ind] * m) -
                   2 * geodist[ind] *
                   ((pow(indclst1_fis[ind], 2) + 2 * indclst1_fis[ind] * indclst2_fis[ind] + pow(indclst2_fis[ind], 2))/4) *
                   exp(-2 * geodist[ind] * m);
        // loss
        mloss = pow( (gendist[ind] - ((indclst1_fis[ind] + indclst2_fis[ind])/2) *
                exp(-m*geodist[ind])), 2);
        // gradient
